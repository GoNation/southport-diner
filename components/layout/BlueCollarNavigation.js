...
const Navigation = ({
  ...
  const styles = {
    ...
    navItemTextStyle: (route, hasScrolled) => ({
      as: 'span',
      color: 'white',
      textTransform: 'uppercase',
      display: 'flex',
    ...
    linkItemStyle: (route, hasScrolled) => ({
      //... styles based on route and hasScrolled
    }),
    flexNavItemsStyle: {
  ...
  const renderRoute = useCallback(
    route =>
      route.dropdownLabel ? null : (
        <Box key={slugifyLower(route.name)} {...styles.navItemBoxStyle}>
          {route.children ? (
            <>
              <Text
                as="span"
